...
export class PubSubManager {
    ...
    private userSockets: Map<string, WebSocket>;  // Maps user IDs to WebSocket connections
    ...
    public userSubscribe(userId: string, stock: string, ws: WebSocket) {
        ...
        if (this.subscriptions.get(stock)?.length === 1) {
            ...
            this.redisClient.subscribe(stock, (message) => {
                this.handleMessage(stock, message, ws);
            });
    ...
    private handleMessage(stock: string, message: string,  ws: WebSocket) {
        console.log(`Message received on channel ${stock}: ${message}`);
        this.subscriptions.get(stock)?.forEach((sub) => {
            const wsConnection =  this.userSockets.get(sub); 
